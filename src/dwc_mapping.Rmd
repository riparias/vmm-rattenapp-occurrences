...
catches_filename <- "Vangsten 2022.xlsx"
# Get all sheets
...
# Merge all catches data
catches <- bind_rows(catches_2021, catches)

# Relocate columns based on cols observations
catches <- 
  catches %>%
  relocate(
...
# Remove observations in catches (same registrationID-species combination)
observations <- 
  observations %>%
...
```{r merge_catches_obs}
occurrences <- bind_rows(observations, catches)
```
