...
# Merge plants and animals observations
...
We got also catches from 2021 apart. We remove from `observations` the observations related to catches (same registration ID and species name) as they are duplicates:

```{r catches_2021}
# catches
catches_filename <- "Vangsten T13_05_14.xlsx"
# Get all sheets
...
catches_2021 <- 
  catches_2021 %>%
  relocate(
    one_of(names(observations)[names(observations) %in% names(catches_2021)])
)

# Remove observations included in catches (same registrationID and species)
observations <- 
  observations %>%
  anti_join(catches_2021 %>% dplyr::select(`Registratie ID`, 
                                           `Sporen Waarnemingen Naam`),
            by = c("Registratie ID", "Sporen Waarnemingen Naam"))
```

We can now proceed by merging `catches_2021` and  `observations`:

```{r merge_catches_obs}
occurrences <- bind_rows(observations, catches_2021)
```
...
and `automatic-update` branch:
 
```{r automatic-update}
dwc_occurrence_update <- readr::read_csv(
  file = "https://raw.githubusercontent.com/riparias/vmm-rattenapp-occurrences/automatic-update/data/processed/occurrence.csv",
  col_types = cols(.default = "c"))
```
...
```{r nrows}
paste("Number of occurrences (new):", nrow(dwc_occurrence_update))
paste("Number of occurrences (old):", nrow(dwc_occurrence_main))
paste("Difference:", nrow(dwc_occurrence_update) - nrow(dwc_occurrence_main))
```

## New `Sporen Waarnemingen Naam` values

The DwC `scientificName` is a manual mapping of Dutch vernacular names (e.g. `Roerdomp`) or expressions (e.g. `Eendensterfte circa 25 st`) in input field `Sporen Waarnemingen Naam`. It is therefore important to get a list with all new values:
