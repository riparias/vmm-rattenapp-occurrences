...
This document describes how (and contains the code to) transform the rattenapp data of the [Flanders Environment Agency](https://en.vmm.be/) (VMM) to a [Darwin Core Archive](https://ipt.gbif.org/manual/en/ipt/2.5/dwca-guide) that can be uploaded to an [IPT](https://ipt.gbif.org/manual/en/ipt/2.5/).
...
required <- c("dplyr", "readr", "purrr", "stringr", "here",
              "glue", "readxl", "DBI", "RSQLite", "digest")
...
library(readr)          # To read and write text files
library(purrr)          # To easily work with functions and vectors
...
observations <- observations %>% 
  mutate(species_name_hash = vdigest (.data$`Sporen Waarnemingen Naam`,
                                      algo="md5"))
```
...
```{r create_db}
rattenapp_db <- DBI::dbConnect(RSQLite::SQLite(), ":memory:")
# Import data
DBI::dbWriteTable(rattenapp_db, "observations", observations)
# Import data to filter out
DBI::dbWriteTable(rattenapp_db, "life_mica_obs", life_mica_obs)
```
...
```{r occurrence}
dwc_occurrence_sql <- glue::glue_sql(readr::read_file(here::here("sql", "dwc_occurrence.sql")), .con = rattenapp_db)
dwc_occurrence <- DBI::dbGetQuery(rattenapp_db, dwc_occurrence_sql)
```
